...
func TestPrimeMST(t *testing.T) {
	var testCases = []testCase{
		{
			...
			mst:    []edge.Edge{edge.NewEdge(1, 7, 0.19), edge.NewEdge(0, 2, 0.26), edge.NewEdge(2, 3, 0.17), edge.NewEdge(4, 5, 0.35), edge.NewEdge(5, 7, 0.28), edge.NewEdge(6, 2, 0.40), edge.NewEdge(0, 7, 0.16)},
			weight: 1.81,
		},
	}
	...
	for _, tc := range testCases {
		...
		// Need to cast to float32 due to floating point precision error
		if tc.weight != float32(mst.Weight()) {
